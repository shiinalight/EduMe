...
import database
from database import Row, connect as database_connection
from heygen_video import VIDEO_SCHEMA_SQL, create_video_router
import math_capture
from capture_library import (
    IMPORTED_FOUNDATION, IMPORTED_GUIDANCE, MAX_SAVED_STEPS, UNGRADED_NOTICE,
    LIBRARY_SCHEMA_SQL, Text, create_library_router, create_private_route, imported_question,
    public_source_url,
)
...
SCHEMA_SQL = """
CREATE TABLE IF NOT EXISTS edume_private.students (
...
CREATE TABLE IF NOT EXISTS edume_private.login_sessions (
    token TEXT PRIMARY KEY,
    student_id BIGINT NOT NULL REFERENCES edume_private.students(id),
    created_at BIGINT NOT NULL
);
CREATE INDEX IF NOT EXISTS login_sessions_student ON edume_private.login_sessions(student_id);
CREATE TABLE IF NOT EXISTS edume_private.practice_sessions (
    id TEXT PRIMARY KEY,
    student_id BIGINT NOT NULL REFERENCES edume_private.students(id),
...
CREATE INDEX IF NOT EXISTS practice_sessions_student ON edume_private.practice_sessions(student_id);
CREATE TABLE IF NOT EXISTS edume_private.practice_steps (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    practice_session_id TEXT NOT NULL REFERENCES edume_private.practice_sessions(id),
...
CREATE INDEX IF NOT EXISTS practice_steps_session ON edume_private.practice_steps(practice_session_id, id);
CREATE TABLE IF NOT EXISTS edume_private.student_tutor_preferences (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY,
    student_id BIGINT NOT NULL REFERENCES edume_private.students(id),
    mode TEXT NOT NULL,
    PRIMARY KEY (student_id, mode)
);
CREATE TABLE IF NOT EXISTS edume_private.tutor_strategy_events (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    student_id BIGINT NOT NULL REFERENCES edume_private.students(id),
...
CREATE INDEX IF NOT EXISTS tutor_strategy_events_student ON edume_private.tutor_strategy_events(student_id, foundation);
CREATE TABLE IF NOT EXISTS edume_private.session_tutor_state (
    practice_session_id TEXT PRIMARY KEY REFERENCES edume_private.practice_sessions(id),
...
def initialize_database() -> None:
    """Create the private schema and every table. Run via ``python init_db.py``, never on import."""

    database.initialize(SCHEMA_SQL, LIBRARY_SCHEMA_SQL, VIDEO_SCHEMA_SQL)
...
def student_profile(row: Row) -> StudentProfile:
...
def preferred_tutor_modes(student_id: int) -> list[TutorMode]:
    with database_connection() as connection:
        rows = connection.execute(
            "SELECT mode FROM edume_private.student_tutor_preferences WHERE student_id = %s ORDER BY id",
            (student_id,),
        ).fetchall()
...
def authenticated_student(session_token: str | None) -> Row:
    if not session_token:
        raise HTTPException(status_code=401, detail="Please sign in to begin a practice session.")
    with database_connection() as connection:
        student = connection.execute(
            """
            SELECT students.* FROM edume_private.students
            JOIN edume_private.login_sessions ON login_sessions.student_id = students.id
            WHERE login_sessions.token = %s
            """,
            (session_token,),
        ).fetchone()
    if student is None:
...
def adaptive_tutor_guidance(
    ...
    with database_connection() as connection:
        repeated_errors = connection.execute(
            """
            SELECT COUNT(*) AS count FROM edume_private.practice_steps
            JOIN edume_private.practice_sessions ON practice_sessions.id = practice_steps.practice_session_id
            WHERE practice_sessions.student_id = %s AND practice_steps.error_type = %s
            """,
            (student_id, error_type),
        ).fetchone()["count"] if error_type else 0
        performance = connection.execute(
            """
            SELECT mode, AVG(outcome)::float8 AS success_rate, COUNT(*) AS attempts
            FROM edume_private.tutor_strategy_events
            WHERE student_id = %s AND foundation = %s
            GROUP BY mode
            """,
...
def active_tutor_mode(learning_session_id: str) -> TutorMode:
    with database_connection() as connection:
        row = connection.execute(
            "SELECT active_mode FROM edume_private.session_tutor_state WHERE practice_session_id = %s",
            (learning_session_id,),
        ).fetchone()
...
def set_active_tutor_mode(learning_session_id: str, mode: TutorMode) -> None:
    with database_connection() as connection:
        connection.execute(
            """
            INSERT INTO edume_private.session_tutor_state (practice_session_id, active_mode) VALUES (%s, %s)
...
def record_strategy_outcome(student_id: int, foundation: str, mode: TutorMode, accepted: bool) -> None:
    ...
    with database_connection() as connection:
        connection.execute(
            "INSERT INTO edume_private.tutor_strategy_events (student_id, foundation, mode, outcome, created_at) VALUES (%s, %s, %s, %s, %s)",
...
async def unsavable_value(request, exc) -> JSONResponse:
    """PostgreSQL text cannot hold NUL characters (SQLite accepted them); treat those as bad input."""

    return JSONResponse({"detail": "The request contains a value that cannot be saved."}, status_code=422)
    ...
    try:
        with database_connection() as connection:
            student_id = connection.execute(
                """
                INSERT INTO edume_private.students (full_name, email, password_salt, password_hash, grade, country, state, created_at)
            ...
            connection.execute(
                "INSERT INTO edume_private.login_sessions (token, student_id, created_at) VALUES (%s, %s, %s)",
                (token, student_id, int(time.time())),
            )
            with connection.cursor() as cursor:
                cursor.executemany(
                    "INSERT INTO edume_private.student_tutor_preferences (student_id, mode) VALUES (%s, %s)",
                    [(student_id, mode) for mode in dict.fromkeys(request.tutorModes)],
                )
            student = connection.execute("SELECT * FROM edume_private.students WHERE id = %s", (student_id,)).fetchone()
...
def login_student(request: LoginRequest, response: Response) -> StudentProfile:
    with database_connection() as connection:
        student = connection.execute("SELECT * FROM edume_private.students WHERE email = %s", (request.email.strip().lower(),)).fetchone()
...
def logout_student(response: Response, math_tutor_session: str | None = Cookie(default=None)) -> Response:
    if math_tutor_session:
        with database_connection() as connection:
            connection.execute("DELETE FROM edume_private.login_sessions WHERE token = %s", (math_tutor_session,))
...
def check_learning_step(
    ...
    with database_connection() as connection:
        session = connection.execute(
            "SELECT * FROM edume_private.practice_sessions WHERE id = %s AND student_id = %s",
    ...
    if session["problem_key"].startswith("imported:"):
        ...
        # outcomes. Lock the session row so concurrent steps serialize the counter
        # update and insert in one transaction.
        with database_connection() as connection:
            session = connection.execute(
                "SELECT * FROM edume_private.practice_sessions WHERE id = %s AND student_id = %s FOR UPDATE",
                (learning_session_id, student["id"]),
            ).fetchone()
            if session is None:
            ...
            connection.execute(
                "INSERT INTO edume_private.practice_steps (practice_session_id, step_index, raw_latex, status, error_type, created_at) VALUES (%s, %s, %s, 'unclear', 'ungraded', %s)",
                (learning_session_id, next_step, request.rawLatex, int(time.time())),
            )
            connection.execute("UPDATE edume_private.practice_sessions SET next_step = %s WHERE id = %s", (next_step + 1, learning_session_id))
    ...
    with database_connection() as connection:
        connection.execute(
            "INSERT INTO edume_private.practice_steps (practice_session_id, step_index, raw_latex, status, error_type, created_at) VALUES (%s, %s, %s, %s, %s, %s)",
            (learning_session_id, next_step, request.rawLatex, result.status, result.errorType, int(time.time())),
        )
        if result.stepAccepted:
            connection.execute("UPDATE edume_private.practice_sessions SET next_step = %s WHERE id = %s", (result.nextStep, learning_session_id))
    return result
...
def review_learning_session(
    ...
    with database_connection() as connection:
        ...
        incorrect_steps = connection.execute(
            """
            SELECT step_index, raw_latex, error_type FROM edume_private.practice_steps
            WHERE practice_session_id = %s AND (%s OR status != 'correct')
            ORDER BY id
            """,
...
async def explain_review_step(
    ...
    with database_connection() as connection:
        ...
        attempted_step = connection.execute(
            """
            SELECT step_index, error_type FROM edume_private.practice_steps
            WHERE practice_session_id = %s AND raw_latex = %s AND status != 'correct'
